...
class DualCoop(nn.Module):
    ...
    def forward(self, image, cls_id=None):
        ...
        # normalize features
        text_features = text_features / text_features.norm(dim=-1, keepdim=True) # [96, 1024]
        image_features_norm = image_features / image_features.norm(dim=1, keepdim=True) # [32, 1024, 50]
        
        # Class-Specific Region Feature Aggregation
        output = 20 * F.conv1d(image_features_norm, text_features[:, :, None]) # [32, 96, 50]
        b, c, _ = output.shape
        output_half = output[:,  c // 2:] # [32, 48, 50]
        w_half = F.softmax(output_half, dim=-1)
        w = torch.cat([w_half, w_half], dim=1)
        output = 5 * (output * w).sum(-1) # [32, 96]
        b, c = output.shape

        # convert the shape of logits to [b, 2, num_class]
        logits = output.resize(b, 2, c//2) # [32, 2, 48]
