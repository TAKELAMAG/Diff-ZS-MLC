...
class CocoDetection(datasets.coco.CocoDetection):
    def __init__(self, root, data_split, img_size=224, p=1, annFile="", label_mask=None, partial=1+1e-6):
        ...
        self.root = root
        if annFile == "":
            annFile = os.path.join(self.root, 'synthetic_two_label_200_annotations', 'instances_%s.json' % data_split)
            cls_id = list(range(len(self.classnames)))
        else:
            cls_id = pickle.load(open(os.path.join(self.root, 'synthetic_two_label_200_annotations', "cls_ids.pickle"), "rb"))
            if 'synthesis' in annFile:
                cls_id = cls_id['train'] | cls_id['test']
            elif 'synthetic' in annFile:
                cls_id = cls_id['test']
            else:
        ...
        if self.data_split == "synthetic_two_label_200_images":
