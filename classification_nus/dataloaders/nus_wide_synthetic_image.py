...
class NUSWIDE_ZSL(data.Dataset):
    def __init__(self, root, data_split, img_size=224, p=1, annFile="", label_mask=None, partial=1+1e-6):
        ...
        ann_file_names = {'train': 'formatted_syn_labels_filtered.npy'}
        img_list_name = {'train': 'formatted_syn_images_filtered.npy'}
        self.root = root
        ...
        if annFile == "":
            annFile = os.path.join(self.root, 'syn_annotations', ann_file_names[data_split])
        else:
            raise NotImplementedError
        cls_id = pickle.load(open(os.path.join(self.root, 'syn_annotations', "cls_id.pickle"), "rb"))
        if data_split == 'train':
        ...
        #    cls_id = list(range(1006))
            ...
            raise ValueError
        self.cls_id = cls_id
        image_list = os.path.join(self.root, 'syn_annotations', img_list_name[data_split])
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.root, 'syn_images', self.image_list[img_id].strip())
